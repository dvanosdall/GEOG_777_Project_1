...
def load_raster(raster_path):
    ...
    Returns:
    --------
    array, affine, crs : tuple
        Raster data, transform, and coordinate system
    """
...
def extract_zonal_stats(tracts, raster_path):
    ...
    stats = zonal_stats(
        ...
        nodata=-9999,
        all_touched=False  # Only cells with center inside polygon
    )

    # Add to dataframe
    tracts = tracts.copy()
    tracts['nitrate_mean'] = [s['mean'] if s['mean'] is not None else np.nan for s in stats]
    tracts['nitrate_min'] = [s['min'] if s['min'] is not None else np.nan for s in stats]
    tracts['nitrate_max'] = [s['max'] if s['max'] is not None else np.nan for s in stats]
    tracts['nitrate_std'] = [s['std'] if s['std'] is not None else np.nan for s in stats]
    tracts['nitrate_count'] = [s['count'] if s['count'] is not None else 0 for s in stats]

    # Check for missing values
    missing = tracts['nitrate_mean'].isna().sum()
    if missing > 0:
        print(f"   WARNING: {missing} tracts have no raster coverage")

    print(f"   Nitrate statistics:")
    print(f"     Mean: {tracts['nitrate_mean'].mean():.2f} mg/L")
    print(f"     Min: {tracts['nitrate_mean'].min():.2f} mg/L")
    print(f"     Max: {tracts['nitrate_mean'].max():.2f} mg/L")
    print(f"     Std: {tracts['nitrate_mean'].std():.2f} mg/L")

    return tracts


def create_choropleth(tracts, output_path, column='nitrate_mean'):
    """
    ...
    # Plot tracts colored by nitrate
    tracts.plot(
        column=column,
        ax=ax,
        cmap='RdYlGn_r',
...
def create_summary_stats(tracts, output_path):
    ...
    # Histogram of nitrate values
    ax = axes[0, 0]
    tracts['nitrate_mean'].hist(bins=30, ax=ax, edgecolor='black', alpha=0.7)
    ax.set_xlabel('Mean Nitrate (mg/L)')
    ax.set_ylabel('Number of Tracts')
    ax.set_title('Distribution of Tract-Level Nitrate Exposure')
    ax.axvline(tracts['nitrate_mean'].mean(), color='red', linestyle='--', label='Mean')
    ax.axvline(tracts['nitrate_mean'].median(), color='blue', linestyle='--', label='Median')
    ax.legend()

    # Boxplot
    ax = axes[0, 1]
    tracts.boxplot(column='nitrate_mean', ax=ax)
    ax.set_ylabel('Mean Nitrate (mg/L)')
    ax.set_title('Nitrate Exposure Summary')
    ax.set_xticklabels(['Census Tracts'])

    # Scatter: tract area vs nitrate
    ax = axes[1, 0]
    tracts['area_km2'] = tracts.geometry.area / 1e6
    ax.scatter(tracts['area_km2'], tracts['nitrate_mean'], alpha=0.5)
    ax.set_xlabel('Tract Area (km²)')
    ax.set_ylabel('Mean Nitrate (mg/L)')
    ax.set_title('Tract Area vs Nitrate Exposure')

    # Scatter: pixel count vs nitrate std
    ax = axes[1, 1]
    ax.scatter(tracts['nitrate_count'], tracts['nitrate_std'], alpha=0.5)
